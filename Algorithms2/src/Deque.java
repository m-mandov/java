 ...
 *
 * An industrial strength solution would need to check to see if an iterator had
 * begun to yield values, but then the deque was modified before emptying. This
 * could be accomplished with an instance-local switch that gets turned on when
 * an iterator is "open", turned off when it's empty, and detected before
 * mutating methods are called.
 */
public class Deque<Item> implements Iterable<Item> {
	 ...
	 * inFirst is true if items go in by addFirst, false for addLast inFirst is
	 * true if items come out by removeFirst, false for removeLast return true
	 * if test passes
	 */
